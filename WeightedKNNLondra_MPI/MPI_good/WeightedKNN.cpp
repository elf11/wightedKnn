					...
					TRAINING_EXAMPLES_LIST *testList, int testExamples)
{
	...
	/* Learn weights by cross validation (3 fold) on training set */
		accuracy = CrossValidate (trainList, no_of_iterations, 
 ...
 * to test the KNN.
 */
int thread_code_testknn(void *args)
{
			 ...
			 bool isBackwardElimination, bool isAttWKNN)
{
	...
	int nrIter = ((int)data.size()) / size;
	int remain = ((int)data.size()) - nrIter * size;

	th_knn = (struct testknn *)malloc(sizeof(struct testknn)); 
	...
	if (rank == size - 1) {
		mypart = nrIter + remain;
	} else {
		mypart = nrIter;
	}
	th_knn->tlist =	tlist;
	th_knn->data = data.begin() + rank * mypart;
	th_knn->num = mypart;
	th_knn->isInstanceWeighted = isInstanceWeighted;
	th_knn->mode = mode;
	th_knn->isBackwardElimination = isBackwardElimination;
	th_knn->isAttWKNN = isAttWKNN;
	th_knn->index = index;

	if(rank == 0) {
		correctlyClassifiedInstances = thread_code_testknn(th_knn);
		for(i = 0; i < size-1; i++) {
			MPI_Recv(&rez, 1, MPI_INT, i+1, tag, MPI_COMM_WORLD, &status);
			correctlyClassifiedInstances += rez;
		}
	}
	else {
		int result = thread_code_testknn(th_knn);
					...
					bool isAttWeightedKNN)	   //Attribute or Instance Weighted
{
	...
	int nrIterations = ((int)data.size()) / size;
	int remain = ((int)data.size()) - nrIterations * size;

	th_iters = (struct iters *)malloc(sizeof(struct iters));
	
	mypart = (rank == size - 1) ? nrIterations + remain : nrIterations;
	    
    th_iters->tlist = tlist;
    th_iters->data = data.begin() + rank * mypart;
    th_iters->num = mypart;
    th_iters->mode = mode;
    th_iters->isAttWeightedKNN = isAttWeightedKNN;
    th_iters->index = index;
