...
#include "TrainingExample_pthread.h"
...
struct iters {
	TRAINING_EXAMPLES_LIST *tlist;
	TRAINING_EXAMPLES_LIST::iterator data;
	int num;
	MODE mode;
	bool isAttWeightedKNN;
	uint *index;
...
struct iters *th_iters;

struct testknn {
	TRAINING_EXAMPLES_LIST *tlist;
	TRAINING_EXAMPLES_LIST::iterator data;
	int num;
	bool isInstanceWeighted;
	MODE mode;
	bool isAttWeightedKNN;
	bool isBackwardElimination;
	bool isAttWKNN;
	uint *index;
};

struct testknn *th_knn;
...
/*
 * Function used to get the parameters for the threads and then
 * to test the KNN.
 */
void *thread_code_testknn(void *args)
{
	TRAINING_EXAMPLES_LIST::iterator testIter;
	struct testknn *p;
	int *correctlyClassifiedInstances = (int *)calloc(1, sizeof(int));
	int c = 0;
	TrainingExample tmpTestObj;

	p = (struct testknn*)args;
	int k = p->num;

	for (testIter = p->data; k > 0; testIter++) {
		tmpTestObj = *testIter;
		int predictedClass = PredictByKNN(p->tlist, tmpTestObj.Value, p->isInstanceWeighted, p->index, p->mode, p->isBackwardElimination, p->isAttWKNN);
	...
	*correctlyClassifiedInstances = c;
			 ...
			 bool isBackwardElimination, bool isAttWKNN)
{
	...
	TRAINING_EXAMPLES_LIST::iterator testIter;
	uint *index = new uint[K];
	int i, rc;

	int nrIter = ((int)data.size()) / NUM_THREADS;
	int remain = ((int)data.size()) - nrIter * NUM_THREADS;

	pthread_t th_id[NUM_THREADS];

	th_knn = (struct testknn *)malloc(NUM_THREADS * sizeof(struct testknn)); 
	...
	for (i = 0; i < NUM_THREADS; i++) {
		...
		th_knn[i].tlist = tlist;
		th_knn[i].data = data.begin() + i * size;
		th_knn[i].num = size;
		th_knn[i].isInstanceWeighted = isInstanceWeighted;
		th_knn[i].mode = mode;
		th_knn[i].isBackwardElimination = isBackwardElimination;
		th_knn[i].isAttWKNN = isAttWKNN;
		th_knn[i].index = index;
	}
	...
	/*
 	 ...
 	 */
	for (i = 0; i < NUM_THREADS; i++) {
		if ((rc = pthread_create(&th_id[i], NULL, thread_code_testknn, &th_knn[i])) != 0) {
	...
	/*
 	 * Collect the results after the thread finished.
 	 */ 
	for (i = 0; i < NUM_THREADS; i++) {
		if ((rc = pthread_join(th_id[i], &rez[i])) != 0) {
			printf("Couldn't finish!\n");
		}
	}


	for (i = 0; i < NUM_THREADS; i++) {
		correctlyClassifiedInstances += *(int *)rez[i];
		free(rez[i]);
	}
				  ...
				  bool isBE, bool isAttWeightedKNN)
{
	...
	sort(elistWithD.begin(), elistWithD.end(), compare);
...
void* thread_function(void *args)
{
    ...
    for(iter = p->data ; k > 0 ; ++iter)
    {
	TrainingExample TEObj = *iter;
	
	int predictedClass = PredictByKNN (p->tlist, TEObj.Value, true, p->index, p->mode, false, p->isAttWeightedKNN);
	...
	if(actualClass != predictedClass)
	{
		...
		AdjustWeightsByGradientDescent (TEObj.Value, p->tlist, error, p->index, p->isAttWeightedKNN);
	}
	k--;
    }
					...
					bool isAttWeightedKNN)	   //Attribute or Instance Weighted
{
	...
	int nrIterations = ((int)data.size()) / NUM_THREADS;
	int remain = ((int)data.size()) - nrIterations * NUM_THREADS;
	
	pthread_t th_id[NUM_THREADS];
	th_iters = (struct iters *)malloc(NUM_THREADS * sizeof(struct iters));
	
	for (i = 0 ; i < NUM_THREADS ; i++)
	{
	    int size = (i == NUM_THREADS - 1) ? nrIterations + remain : nrIterations;
	    
	    th_iters[i].tlist = tlist;
	    th_iters[i].data = data.begin() + i * size;
	    th_iters[i].num = size;
	    th_iters[i].mode = mode;
	    th_iters[i].isAttWeightedKNN = isAttWeightedKNN;
	    th_iters[i].index = index;
	}
	...
	while(tmp!=iterations)
	{
		...
		for (i = 0; i < NUM_THREADS; i++)
		{
		    if ((rc = pthread_create(&th_id[i], NULL, thread_function, &th_iters[i])) != 0)
		    {
			printf("Couldn't create the thread!\n");
			exit(-1);
		    }
		...
		for (i = 0; i < NUM_THREADS; i++)
		{
		    if ((rc = pthread_join(th_id[i], NULL)) != 0)
		    {
			printf("Couldn't finish the thread!\n");
		    }
		}
